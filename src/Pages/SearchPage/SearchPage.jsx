  ...
  return (
    ...
    <div className="pt-4 px-32 min-h-screen bg-gray-50 dark:bg-gray-900">
      <h1 className="text-xl font-bold mb-4 text-gray-900 dark:text-gray-100">
        Hasil Pencarian: <span className="text-green-600 dark:text-green-400">"{title}"</span>
      </h1>

      {/* [PERBAIKAN] Menambahkan kelas dark: pada teks status */}
      {loading && <p className="text-gray-600 dark:text-gray-400">Loading...</p>}
      {error && <p className="text-red-600 dark:text-red-400">{error}</p>}
      {!loading && !error && results.length === 0 && <p className="text-gray-600 dark:text-gray-400">Tidak ada hasil ditemukan.</p>}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {results.map((news) => (
          <div
            key={news.id_news}
            onClick={() => navigate(`/searchdetail/${news.id_news}`, { state: { article: news } })}
          ...
          >
            ...
            />
            <h2 className="font-semibold text-lg mb-1 text-gray-900 dark:text-gray-100">{news.title}</h2>
            <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">
              {news.create_at
                ? new Date(news.create_at).toLocaleDateString('id-ID', { day:'numeric', month: 'long', year:'numeric' })
                : 'Tanggal tidak tersedia'}
            </p>
            <p className="text-sm text-gray-600 dark:text-gray-300 line-clamp-3 flex-grow">{news.description}</p>
