...
const CategoryPage = () => {
  ...
  useEffect(() => {
    const fetchAllArticles = async () => {
      ...
      try {
        const dbRes = await fetch(`${API_BASE_URL}/api/news/category/${categoryName}`)
  ...
  const handleArticleClick = article => {
    if (article.source === 'api') {
      navigate(`/newsdetail/${encodeURIComponent(article.url)}`, { state: { article } })
    } else {
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {articles.map((article, i) => (
            ...
            >
              ...
              />
              <h3 className="text-lg font-semibold mt-2 text-gray-900 dark:text-gray-100">{article.title}</h3>
