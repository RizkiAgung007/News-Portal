...
import { FaSun, FaMoon, FaArrowDown, FaArrowUp, FaEye } from "react-icons/fa";
...
const UserManagement = () => {
  ...
  const [activePopVisible, setActivePopVisible] = useState(null);
  const popRef = useRef();
  ...
  const fetchUsers = async (page = 1, order = "desc", search = "") => {
    ...
    try {
      const res = await axios.get(
        `${API_BASE_URL}/api/auth/all-users?page=${page}&limit=20&sortOrder=${order}&search=${encodeURIComponent(
          search
        )}`,
        {
  ...
  const tooglePop = (id) => {
    setActivePopVisible(activePopVisible === id ? null : id);
  };

  useEffect(() => {
    const handleClickOutside = (e) => {
      if (popRef.current && !popRef.current.contains(e.target)) {
        setActivePopVisible(null);
      }
    };
  ...
  const confirmDelete = async () => {
    ...
    try {
      await axios.delete(
        `${API_BASE_URL}/api/auth/delete-user/${userToDeleteId}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const navigateToViewUser = (userId) => {
    setActivePopVisible(null);
  ...
  return (
    <div className="p-6 min-h-screen">
                ...
                >
                  ...
                  <td className="px-5 py-3 border-b border-gray-200 dark:border-gray-600 text-center relative">
                    ...
                    <div className="hidden md:flex justify-center items-center space-x-2">
                      <button
                        onClick={() =>
                          handleDeleteClick(user.id_users, user.username)
                        }
                      ...
                      <button
                        onClick={() => navigateToViewUser(user.id_users)}
                    ...
                    <div className="md:hidden flex justify-center items-center">
                      <button
                        onClick={() => tooglePop(user.id_users)}
                      ...
                      {/* Popover / Dropdown Aksi */}
                      {activePopVisible === user.id_users && (
                        <div
                          ref={popRef} 
                          className="absolute left-1/2 -translate-x-1/2 mt-2 w-14 bg-white dark:bg-gray-800 rounded-md shadow-xl z-10 overflow-hidden border border-gray-200 dark:border-gray-700"
                        >
                          <button
                            onClick={() => navigateToViewUser(user.id_users)}
                          ...
                          <button
                            ...
                            // PERBAIKAN STYLING TOMBOL DALAM POPOVER
                            className="w-full text-left px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center space-x-2"
                            disabled={loading}
                          >
      ...
      {/* Confirm Modal untuk Delete User */}
      <Confirm
        ...
        message={`Are you sure you want to delete the user "${userToDeleteUsername}"? Tihs action cannot be undone.`}
